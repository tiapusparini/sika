<?php 
	...
	class m_peserta_data_bank extends CI_Model {
		...
		{
			$q = $this->db->select('*')->from('peserta_data_bank')->where('kode_data_bank',$xid)->get();
		...
		{
			$this->db->insert('peserta_data_bank',$data);
		}
		...
		{
			$this->db->where('kode_data_bank',$xid);
			$this->db->update('peserta_data_bank',$data);
		}
