<?php 
	...
	class m_jenis_peserta extends CI_Model {
		...
		{
			$q = $this->db->select('*')->from('jenis_peserta')->where('kode_jenis_peserta',$xid)->get();

			return $q->result();
		}

		public function getGroupStatusKeluarga($xid)
		{
			$q = $this->db->get_where('jenis_peserta', array('kode_jenis_peserta' => $xid));
		...
		{
			$this->db->insert('jenis_peserta',$data);
		}
